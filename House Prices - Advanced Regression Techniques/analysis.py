...
# sns.heatmap(pearson_corr, vmax=.8, square=True)
...
# cols = pearson_corr.nlargest(k, 'SalePrice')['SalePrice'].index
# print(cols)
...
#
# plt.figure(figsize=(16, 9), dpi=300)
# cols = df_train.corr(method='spearman').nlargest(k, 'SalePrice')['SalePrice'].index
# print(cols)
# cm = numpy.corrcoef(df_train[cols].values.T)
# sns.set(font_scale=1.25)
...
pass
# # region SalePrice分布特性
# # 查看SalePrice分布特性
# fig1 = plt.figure()
# (mu, sigma) = norm.fit(df_train['SalePrice'])
# sns.displot(df_train['SalePrice'], kde=True, height=10, aspect=1.5)
# plt.legend(['正态曲线拟合（$\mu=${:.2f}、$\sigma=${:.2f}）'.format(mu, sigma)], loc='best')
# plt.ylabel('数量')
# plt.title('SalePrice柱状图')
# plt.show()
#
# # 绘制QQ图
...
#
# # 使用log1p对数据做正态转换
# df_train["SalePrice"] = numpy.log1p(df_train["SalePrice"])
#
# # 查看SalePrice的正态性
# fig3 = plt.figure()
# (mu, sigma) = norm.fit(df_train['SalePrice'])
